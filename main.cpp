...
int main() {
  ...
  cube_mesh.rotate_y(30.0f);
  cube_mesh.scale({0.2, 0.5, 0.2});
  cube_mesh.translate({-0.3, -0.25, 0.5});

  Mesh roof = Mesh({
    Triangle({-0.5f, -0.5, 2.0f},{0.5f, -0.5, 2.0f},{-0.5f,-0.5, 4.0f},{0, {255,255,255}, 0, 0}, {0,1,0}),
    Triangle({-0.5f, -0.5, 4.0f},{0.5f, -0.5, 2.0f},{0.5f, -0.5, 4.0f},{0, {255,255,255}, 0, 0}, {0,1,0}),

    Triangle({-0.5f, 0.5, 2.0f},{-0.5f, 0.5, 4.0f},{-0.5f,-0.5, 4.0f},{0, {255,0,0}, 0, 0}, {1,0,0}),
    Triangle({-0.5f, -0.5, 2.0f},{-0.5f, 0.5, 2.0f},{-0.5f, -0.5, 4.0f},{0, {255,0,0}, 0, 0}, {1,0,0}),

    Triangle({0.5f, 0.5, 2.0f},{0.5f, 0.5, 4.0f},{0.5f,-0.5, 4.0f},{0, {0,255,0}, 0, 0}, {-1,0,0}),
    Triangle({0.5f, -0.5, 2.0f},{0.5f, 0.5, 2.0f},{0.5f, -0.5, 4.0f},{0, {0,255,0}, 0, 0}, {-1,0,0}),

    Triangle({-0.5f, -0.5, 4.0f},{ 0.5f, -0.5, 4.0f},{-0.5f,  0.5, 4.0f},{0, {255,255,255}, 0, 0}, {0,0,-1}),
    Triangle({0.5f, 0.5, 4.0f},{ 0.5f, -0.5, 4.0f},{-0.5f,  0.5, 4.0f},{0, {255,255,255}, 0, 0}, {0,0,-1}),
  ...
  for (int i = 0; i<cube_mesh.triangles.size(); i++) {
    cube_mesh.triangles[i].material.refractive_index = 1.5;

    cube_mesh.triangles[i].material.color[0] = 255;
    cube_mesh.triangles[i].material.color[1] = 255;
    cube_mesh.triangles[i].material.color[2] = 255;
  ...
  vector<Sphere> spheres = {
    // {{-0.2,-0.3,2.5}, 0.20, {0.0, {255, 255,255}, 0.0, 0.0, -1}},
    // {{0.2,-0.3,2.5}, 0.20, {0.0, {255, 255,255}, 0.0, 0.0, -1}},
  };
...
#pragma omp parallel for collapse(2)
  for (int y = 0; y<HEIGHT; y++) {
    for (int x = 0; x<WIDTH; x++) {
      ...
      Ray ray = {camera_pos, direction, 1, camera_pos.z+1, 0};
